...
histogram *get_histogram(pngquant_image *input_image, struct pngquant_options *options)
{
    ...
    */
    ...
    for (; ;) {
        ...
        ignorebits++;
        verbose_printf(options, "  too many colors! Scaling colors to improve clustering...");
...
void prepare_image(pngquant_image *input_image, struct pngquant_options *options)
{
    if (options->min_opaque_val <= 254.f/255.f) {
        verbose_printf(options, "  Working around IE6 bug by making image less transparent...");
...
colormap *pngquant_quantize(histogram *hist, const struct pngquant_options *options)
{
    ...
    if (iterations) {
        verbose_printf(options, "  moving colormap towards local minimum");
